...
int main(int argc, char **argv) {
    ...
    try {
        transformA = tfBuffer.lookupTransform("prbt_tool0", "camera_link", rclcpp::Time(0), rclcpp::Duration::from_seconds(5.0));
        transformB = tfBuffer.lookupTransform("camera_link", "camera_color_optical_frame", rclcpp::Time(0), rclcpp::Duration::from_seconds(5.0));
    ...
    RCLCPP_INFO(node->get_logger(), "camera_link to camera_color_optical_frame: %s", geometry_msgs::msg::to_yaml(transformB).c_str());
    ...
    RCLCPP_INFO(node->get_logger(), "new prbt_tool0 to camera_link: %s", geometry_msgs::msg::to_yaml(transformD).c_str());
